...
Google Cloud Storage). This crate is the implementation library behind the
`snapdir` binary.
...
`snapdir-cli` versions <= 1.5 keep installing the old binary (`cargo install
snapdir-cli@1.5.0` still works). From the next release this crate installs no
binary itself: it exposes `snapdir_cli::run()`, the binary entrypoint the
