...
fn all_flag_includes_unchanged_paths() {
    ...
    assert!(
        all.lines().any(|l| l.ends_with("./drop.txt") && l.starts_with('D')),
        "drop.txt must still be D under --all; got:\n{all}"
    );
    assert!(
        all.lines().any(|l| l.ends_with("./add.txt") && l.starts_with('A')),
...
fn diff_reads_manifests_only_ignores_bogus_objects_pool() {
    ...
    let (from_store, from_url, _fid) = capture(
        "mo-from",
        &cache,
        &[("keep.txt", b"same", 0o644), ("old.txt", b"old only", 0o644)],
    );
...
fn diff_works_with_absent_objects_directory() {
    ...
    let (from_store, from_url, _fid) =
        capture("ao-from", &cache, &[("a.txt", b"one", 0o644)]);
    let (to_store, to_url, _tid) =
        capture("ao-to", &cache, &[("a.txt", b"two-changed", 0o644)]);
...
fn json_output_is_array_of_status_path_objects() {
    ...
    let (_fs, from_url, _fid) = capture(
        "json-from",
        &cache,
        &[("keep.txt", b"same", 0o644), ("gone.txt", b"x", 0o644), ("chg.txt", b"v1", 0o644)],
    );
    let (_ts, to_url, _tid) = capture(
        "json-to",
        &cache,
        &[("keep.txt", b"same", 0o644), ("add.txt", b"y", 0o644), ("chg.txt", b"v2-longer", 0o644)],
    );
    ...
    want.sort();
    assert_eq!(pairs, want, "json entries must match the change set; got:\n{json}");
...
fn multi_ref_same_path_same_content_no_conflict() {
    ...
    let (_f1, from1_url, _f1id) =
        capture("mrs-from1", &cache, &[("dup.txt", b"identical", 0o644), ("a.txt", b"a", 0o644)]);
    let (_f2, from2_url, _f2id) =
        capture("mrs-from2", &cache, &[("dup.txt", b"identical", 0o644), ("b.txt", b"b", 0o644)]);
...
fn intra_side_collision_errors_by_default() {
    ...
    let (_f1, from1_url, _f1id) =
        capture("col-from1", &cache, &[("clash.txt", b"left version", 0o644)]);
    let (_f2, from2_url, _f2id) =
        capture("col-from2", &cache, &[("clash.txt", b"RIGHT version", 0o644)]);
...
fn intra_side_collision_last_wins_selects_last_ref() {
    ...
    let (_ts2, to2_url, _tid2) =
        capture("lw-to2", &cache, &[("clash.txt", b"LEFT loses", 0o644)]);
    let stdout2 = run_ok(
        &[
            "diff",
            "--on-conflict",
...
fn swapping_from_to_flips_added_and_deleted() {
    ...
    let (_fs, left_url, _lid) = capture(
        "dir-left",
        &cache,
        &[("keep.txt", b"k", 0o644), ("leftonly.txt", b"L", 0o644), ("chg.txt", b"v1", 0o644)],
    );
    let (_ts, right_url, _rid) = capture(
        "dir-right",
        &cache,
        &[("keep.txt", b"k", 0o644), ("rightonly.txt", b"R", 0o644), ("chg.txt", b"v2", 0o644)],
    );
    ...
    assert_porcelain_eq(
        &fwd,
        &[("M", "./chg.txt"), ("D", "./leftonly.txt"), ("A", "./rightonly.txt")],
    );
    ...
    assert_porcelain_eq(
        &rev,
        &[("M", "./chg.txt"), ("A", "./leftonly.txt"), ("D", "./rightonly.txt")],
    );
...
fn from_id_pins_single_manifest_in_multi_manifest_store() {
    ...
    let from_url = file_url(&from_store);
    let id_v1 = capture_into("pin-v1", &cache, &from_url, &[("f.txt", b"version one", 0o644)]);
    let id_v2 = capture_into("pin-v2", &cache, &from_url, &[("f.txt", b"version two!!", 0o644)]);
