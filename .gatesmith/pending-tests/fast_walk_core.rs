...
impl Shape {
    ...
    fn materialize(self, dir: &Path) {
        match self {
            ...
            Shape::Mixed => {
                for (name, n) in [("root_a.bin", 16usize), ("root_b.bin", 256), ("root_c.bin", 0)]
                {
...
fn materialize_random_tree(dir: &Path, dir_count: usize, files: &[(usize, usize)]) {
    ...
    for (i, (which, len)) in files.iter().enumerate() {
        ...
        write_file(&target.join(format!("file{i:04}.bin")), &deterministic_bytes(*len));
    }
}
...
fn materialize_small_and_large(dir: &Path) {
    for i in 0..40 {
        write_file(&dir.join(format!("small_{i:03}.bin")), &deterministic_bytes(64));
    }
    for i in 0..3 {
        write_file(
