...
Next gate: `perf-gate` (phase 7, owner bench, **HUMAN CHECKPOINT**, deps bench-compile ✓ + file-store-roundtrip ✓) — head of the priority queue; then `proptest-roundtrip`/`cli-trycmd` (P8), `rustdoc-doctests` (P9).
⚠ `perf-gate`'s `verification_cmd` is `bash benches/compare.sh` — but **that harness does not exist yet** (bench-compile built the criterion micro-benches, not the hyperfine compare script). Like `interop-harness→interop-diff` and `remote-stores-harness→remote-interop`, the next tick should **GATE-ADD `perf-harness` (bench)** to build `benches/compare.sh` (hyperfine Rust-vs-Bash `manifest` on many-small + few-large corpora, JSON report the perf-gate can `json_path`, `--self-check`), THEN run perf-gate for the operator's sign-off. Do NOT escalate perf-gate to the human until the harness exists and has produced real numbers.
Bench runner note (for compare.sh / perf-gate): run criterion via `cargo bench -p snapdir-benches --bench hot_paths` — the crate's `src/lib.rs` default harness otherwise intercepts criterion's CLI flags.
Last passed: `bench-compile` @ 2026-06-01T02:43:59Z — 3 criterion hot-path groups (`hash/blake3/{64B..1M}` Throughput::Bytes; `walk/{many_small,few_large}`; `manifest/{emit,parse}/{1k,10k}`), `tempfile` scratch corpora, deterministic + self-cleaning; smoke-run e.g. blake3/1M ~1.43 GiB/s. Benches only measure (output bytes unchanged). Contract frozen (locks 4/4 OK each tick).
...
- Phase 7 (Performance): 2/3 passed (`bench-scaffold` ✅ `bench-compile` ✅; `perf-gate` remains — needs a `perf-harness` prereq + operator sign-off)
